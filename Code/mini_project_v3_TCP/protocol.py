...
from random_number_generator import generate_random_number  # Still used for task numbers

# TCP CONFIG
TCP_HOST = "127.0.0.1"  # Where the ranking client will run
TCP_PORT = 5000
...
async def handle_ranking_request(ctx: Context, sender: str, _: RequestRanking):
    ...
    ctx.storage.set("state", "busy")  # Lock immediately

    vehicle_number = ctx.storage.get("vehicle_number")
    ranking = get_ranking_from_tcp(ctx)

    if ranking is None:
        ctx.logger.error("Failed to retrieve ranking from TCP client.")
        ctx.storage.set("state", "idle")
        return
    ...
    ctx.storage.set("state", "idle")  # Unlock after sending
...
# ======= TCP Helper =======
def get_ranking_from_tcp(ctx: Context) -> int | None:
    """Connects to TCP server, retrieves random ranking."""
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.connect((TCP_HOST, TCP_PORT))
            s.sendall(b"GET_RANKING")
            data = s.recv(1024)
            return int(data.decode().strip())
    except Exception as e:
        ctx.logger.error(f"TCP ranking fetch failed: {e}")
        return None
