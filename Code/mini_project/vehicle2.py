...
# It runs on port 8002 and listens for messages at the specified endpoint.
vehicle2 = Agent(name="vehicle2", 
            seed="vehicle 2 recovery phrase", 
            port=8002, 
            endpoint=["http://localhost:8002/submit"])

# Assign vehicle number
vehicle_number_local = 2

# Receives a random number
ranking_local = generate_unique_random_number()
print(f"Vehicle 2 got: {ranking_local}")

# Bob handling incoming messages
@vehicle2.on_message(model=Message)
